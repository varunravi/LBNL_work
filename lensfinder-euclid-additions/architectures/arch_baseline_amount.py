...
def dihedral(x, i):
    if len(x.shape) == 3:  # AJPILON this is used for 4 band? not sure what len=4 refers to because even the 4 band images are (101,101,4) which is 3-dim
        if i & 4:
    ...
    if len(x.shape) == 4:  # AJPILON idk when this is used now. list of arrays? who knows
        if i & 4:
...
class CNN:
    ...
    @staticmethod
    def split_test_train(path, n_data):
        # extract the files names and split them into test and train set
        import os
        files = ['{}/{}'.format(path, f) for f in sorted(os.listdir(path))]
        n_test = int(n_data*0.3)  # added by AJP
        # return files[:3000], files[3000:] # commented out by AJP
    ...
    def prepare(images):
        ...
        # elif images.shape[-1] == 3: # AJPILON calculate mean and std of new (3 band) dataset and normalize with that
        #     images = (images - )
        return images
