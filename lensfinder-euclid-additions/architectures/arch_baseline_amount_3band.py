...
def summary_images(x, name):
    # AJPILON this checks if image is 1 or 4 bands, then gets a summary(?) of each band
    for i in range(min(4, x.get_shape().as_list()[3])):
        # AJPILON should still work on 3 bands though. shape would be less than 4?
...
class CNN:
    ...
    def create_architecture(self, bands):
        ...
        # AJPILON hopefully this works with 3 bands
    ...
    def split_test_train(path, n_data, manual=False):
        ...
        if not manual:
            ...
            # AJPILON added this to calculate 30% split amount
        ...
        # if a test set is organized by *all lenses, then all nonlenses*, and you know how much you want into each set, use these:
            ...
            print(
                'train/test split: semi-manual (25/75, 25/75 split for test and train sets)')
            ...
            # 25% of lens to test
            test_lens = all_lens[:int(len(all_lens)*0.25)]
            # 75% of lens to train
            train_lens = all_lens[int(len(all_lens)*0.25):]
            # 25% of nonlens to test
            test_nonlens = all_nonlens[:int(len(all_nonlens)*0.25)]
            # 75% of nonlens to train
            train_nonlens = all_nonlens[int(len(all_nonlens)*0.25):]
            ...
            test_nonlens = files[n_test_lens +
                                 n_train_lens:n_test_lens+n_train_lens+n_test_nonlens]
            train_nonlens = files[n_test_lens+n_train_lens +
                                  n_test_nonlens:n_test_lens+n_train_lens+n_test_nonlens+n_train_nonlens]
            test_set = test_lens + test_nonlens
            train_set = train_lens + train_nonlens
    ...
    def prepare(images):
        ...
        # AJPILON calculate mean and std of new dataset (3 band ground based, all 120,000) and normalize with those values
        elif images.shape[-1] == 3:
            # hardcoded values calculated in get_mean_std.py
