...
def images_to_sprite(data):
    ...
    ## COPYING BELOW CODE: ATTEMPTING 3 BAND FIX
    ...
    if len(data.shape) == 3: # AJPILON I think this refers to 1 image or image array,NOT band number
        data = np.tile(data[...,np.newaxis], (1,1,1,3))
    data = data.astype(np.float32)
    min_ = np.min(data.reshape((data.shape[0], -1)), axis=1) # AJPILON maybe nothing changes. 4 -> 3 band shouldn't change array dimensionality
    data = (data.transpose(1,2,3,0) - min_).transpose(3,0,1,2)
    max_ = np.max(data.reshape((data.shape[0], -1)), axis=1)
    data = (data.transpose(1,2,3,0) / max_).transpose(3,0,1,2)
...
def predict_all(session, CNN, cnn, files, f, step=10): # AJPILON CHANGE STEP (50 -> 12)
...
def main(arch_path, images_path, output_path, n_iter, n_data, output_num):
    time_total_0 = time()
    if os.path.isdir(output_path):
        ...
        output_path = output_path+output_num
        resume = False
    ...
    # for specified split --- enter manual as: n_test_lens, n_train_lens, n_test_nonlens, n_train_nonlens
    files_test, files_train = CNN.split_test_train(images_path, n_data, manual=(26,79,54,161)) # for 320 set (105 lens/215 nonlens)
    ...
    embedding_amount = 80 #AJPILON 1000 -> 80 to work for test (really its validation) set size of 80 (320 total set size)
    ...
    if not os.path.isfile(sprite_path):
        ...
        if bands == 4:
            images = images[:, :, :, :3]
        if bands == 3: # AJPILON
            iamges = images[:, :, :, :2] # AJPILON do I need this? idk maybe try both 2 and 3
    ...
    if not os.path.isfile(tsv_path):
        ...
        for i in range(embedding_amount):
            with np.load(files_test[i]) as data:
                ...
                if is_lens == 0:  # AJPILON bc we won't have these values, need to get it to work without them
                    metadata_file.write('{}\t{}\t{}\t{}\t{}\n'.format(data['id'], is_lens, 0, 0, 0))
                elif is_lens == 1:
                    metadata_file.write('{}\t{}\t{}\t{}\t{}\n'.format(data['id'], is_lens, data['numb_pix_lensed_image'], data['einstein_area'], data['flux_lensed_image_in_sigma']))
                else:
                    print('uh oh')
    ...
    saver = tf.train.Saver(max_to_keep=5000) # AJPILON ADJUST THIS BC WE WANT TO BE ABLE TO TEST AUC FOR PREVIOUS MODELS AND SEE GROWTH
    ...
    def save_statistics(i):
        if (i // 1000) % 3 == 1: 
            ...
            for j in range(0, embedding_amount, 10): # AJPILON 100 -> 10 to work for set size of 80 (320 total)
                f.write('{}/{}\n'.format(j, embedding_amount))
                f.flush()
                data[j: j+10] = session.run(embedding_input_flatten, feed_dict={ cnn.tfx: CNN.load(files_test[j: j+10]) })
        ...
        ps_train, xentropy_train = predict_all(session, CNN, cnn, files_train[:len(files_test)], f) # AJPILON only get predictions for same amount as test data? why it do this (I didn't)
    ...
    def trainer():
        # xentropy_SAVED = 0 # AJP to try and revert back to previous xentropy if 'nan' is returned
        for i in range(resume_iter, resume_iter + n_iter + 1):
            ...
            if i % 100 == 0 and i != 0: #AJP 100 -> 10
                t100_end = time()
                t100_total = t100_end - t100_start
                f.write("\nLast 100 iterations took {} seconds\n".format(str(t100_total)))
            ...
            if i == 102 or i == 1002:
                ...
                if math.isnan(xentropy) or math.isinf(xentropy): # AJP
                    f.write('\nNAN DETECTED BEEP BOOP BEEP BOOP !!!! line 403\n') # AJP
                    print('nan so i exiting')
                    sys.exit('nan so i exit')
                    SAVE_XENTROPY = (xentropy, _) # AJP
                # else: # AJP
                #     xentropy_SAVED = xentropy
                ...
                if math.isnan(xentropy) or math.isinf(xentropy): # AJP
                    f.write('\nNAN DETECTED BEEP BOOP BEEP BOOP !!!! line 414\n') # AJP
                    print('nan so i exiting')
                    sys.exit('nan so i exit')
                    SAVE_XENTROPY = (xentropy, s) # AJP
                # else:
                #     xentropy_SAVED = xentropy
                ...
                if math.isnan(xentropy) or math.isinf(xentropy): # AJP
                    # set_trace() # AJP try to track down this dang xentropy thing
                    f.write('\nNAN DETECTED BEEP BOOP BEEP BOOP !!!! line 420\n') # AJP
                    print('nan so i exiting')
                    sys.exit('nan so i exit')
                    SAVE_XENTROPY = (xentropy, s) # AJP
                # else:
            ...
            if math.isnan(xentropy) or math.isinf(xentropy):
                f.write('BEEP BOOP BEEP BOP, THE PROGRAM FOUND OUT NOW ABOUT THE NAN line 454\n') # AJP
                f.write(str(SAVE_XENTROPY[0])) # AJP
                f.write(str(SAVE_XENTROPY[1])) # AJP
                print('nan so i exiting')
                sys.exit('nan so i exit')
                f.flush() # AJP
                return
...
if __name__ == '__main__':
    main(argv[1], argv[2], argv[3], int(argv[4]), int(argv[5]), '_{}'.format(argv[6]))
