...
def main(arch_path, images_path, output_path, n_iter, n_data):
    time_total_0 = time()
    ...
    # files_train = files_train[:3500] # AJP - restrict data amount? - this was added because my change to the split didn't work
#                                           also the arch path is specified by me so I know it's getting this method from my baseline_amount version                           
    f.write(" Done\n")
    ...
    embedding_amount = 72 #AJPILON 1000 -> 72 to work for set size of 72 (240 total)
    ...
    saver = tf.train.Saver(max_to_keep=200) # AJPILON ADJUST THIS BC WE WANT TO BE ABLE TO TEST AUC FOR PREVIOUS MODELS AND SEE GROWTH
    ...
    def save_statistics(i):
        if (i // 1000) % 3 == 1: 
            ...
            for j in range(0, embedding_amount, 12): # AJPILON 100 -> 12 to work for set size of 72 (240 total)
                f.write('{}/{}\n'.format(j, embedding_amount))
                f.flush()
                data[j: j+12] = session.run(embedding_input_flatten, feed_dict={ cnn.tfx: CNN.load(files_test[j: j+12]) })
...
if __name__ == '__main__':
    main(argv[1], argv[2], argv[3], int(argv[4]), int(argv[5]))
